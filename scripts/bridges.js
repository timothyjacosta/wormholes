/* GENERATED from scripts/modules/bridges-controller.mjs. Do not edit this direct-file compatibility adapter. */
...
function requestBridgeRemoval(sourceLabel, targetLabel, onConfirm) {
  return (globalThis.controllerServices || globalThis).openRelationshipRemovalConfirm(
    "bridge",
    sourceLabel,
    targetLabel,
    onConfirm,
...
function updateDestructiveClearButtons() {
  setDestructiveButtonVisibility(
    "clearConnectionsBtn",
    (globalThis.controllerServices || globalThis).currentUniverseHasClearableConnections(),
  );
...
function targetCreationIdsForBridgeNode(universeId, creationId) {
  ...
  if ((globalThis.controllerServices || globalThis).isGroupEntry(entry)) {
    (globalThis.controllerServices || globalThis).groupChildIds(entry).forEach((childId) => ids.push(childId));
  }

  return (globalThis.controllerServices || globalThis).uniqueList(ids)
    .filter((id) => (globalThis.controllerServices || globalThis).entityExistsInUniverse(universeId, id, archive));
}
...
function addBridgeToEntryTarget(sourceUniverseId, sourceCreationId, target) {
  ...
  if (
    targetCreationId &&
    !(globalThis.controllerServices || globalThis).entityExistsInUniverse(target.universeId, targetCreationId)
  )
    return false;
  ...
  if (sourceUniverseId === currentUniverseId) (globalThis.controllerServices || globalThis).renderArchive();
...
function toggleEntryBridgeToExternalNode(sourceCreationId, externalNodeId) {
  ...
  const target = (globalThis.controllerServices || globalThis).parseConnectionsExternalNodeId(externalNodeId);
    ...
    if (existing) {
      if (options.confirmRemoval === true) {
        requestBridgeRemoval(
          ...
          (globalThis.controllerServices || globalThis).connectionsMapNodeTitle(externalNodeId),
          () =>
...
function removeMapBridgeToExternalNode(sourceCreationId, externalNodeId, options = {}) {
  ...
  if (target.type === "universe") {
    ...
    if (record) {
      if (options.confirmRemoval === true) {
        requestBridgeRemoval(
          (globalThis.controllerServices || globalThis).getEntry(sourceCreationId)?.title || "Selected item",
          (globalThis.controllerServices || globalThis).connectionsMapNodeTitle(externalNodeId),
          () =>
  ...
  return (universe.bridges || [])
    ...
    .filter(
      (bridge) =>
        ...
        (globalThis.controllerServices || globalThis).linkBridgeTargetStillExists(bridge),
    );
}
...
function toggleUniverseBridge(
  ...
  if (directRemoval && options.confirmRemoval === true) {
    const targetLabel = targetCreationId
      ? (globalThis.controllerServices || globalThis).getCreationTitleFromUniverse(targetUniverseId, targetCreationId)
...
function normalizeBridges(bridges) {
  return (bridges || [])
    .map(normalizeBridge)
    .filter((bridge) => bridge && (globalThis.controllerServices || globalThis).linkBridgeTargetStillExists(bridge));
}
...
function entryPickerMeta(entry) {
  if ((globalThis.controllerServices || globalThis).isGroupEntry(entry)) return "Group";
...
function renderBridgeUniverseList() {
  ...
  const entry = (globalThis.controllerServices || globalThis).getEntry(activeBridgeEntryId);
  ...
  list.innerHTML = universes
    .map((universe) => {
      ...
      const creationCount = archive.filter((item) => !(globalThis.controllerServices || globalThis).isGroupEntry(item)).length;
      const groupCount = archive.filter((item) => (globalThis.controllerServices || globalThis).isGroupEntry(item)).length;
...
function saveBridgePickerModal() {
  ...
  const nextBridges = Array.from(stagedBridgeTargetKeys)
    ...
    .filter(
      (bridge) => bridge.universeId && (globalThis.controllerServices || globalThis).linkBridgeTargetStillExists(bridge),
    );
...
function createBridgeNewUniverse() {
  ...
  if (!title) {
    (globalThis.controllerServices || globalThis).setModalErrorText("bridgeNewUniverseError", "A universe title is required.");
    input.focus();
    return;
  }
  ...
  if ((globalThis.controllerServices || globalThis).duplicateUniverseTitleExists(title)) {
  ...
  universe.diskFolderName = (globalThis.controllerServices || globalThis).stableUniverseFolderName(universe);
...
function openWormholesModal() {
  ...
  (globalThis.controllerServices || globalThis).cleanupAllStaleLinks();
...
function toggleWormholeInternalConnection(universeId, sourceId, targetId, options = {}) {
  ...
  if (alreadyConnected) {
    ...
    const noteKey = (globalThis.controllerServices || globalThis).makeConnectionKeyFromIds(sourceId, targetId);
...
function toggleWormholeBridge(
  ...
  if (targetCreationId) {
    ...
    if (existing) {
      if (options.confirmRemoval === true) {
        requestBridgeRemoval(
          (globalThis.controllerServices || globalThis).getCreationTitleFromUniverse(sourceUniverseId, sourceId),
          (globalThis.controllerServices || globalThis).getCreationTitleFromUniverse(targetUniverseId, targetCreationId),
          () => {
      ...
      if (removed) (globalThis.controllerServices || globalThis).renderWormholesMap();
      return removed;
    }
  } else {
...
(globalThis.registerControllerServices || (() => {}))(BRIDGES_CONTROLLER_API);
