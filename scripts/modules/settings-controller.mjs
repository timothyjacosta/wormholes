...
function preserveSafeExternalLink(element) {
  ...
  if (!anchor || controllerServices.isWormholesSafeDownloadElement(anchor)) return false;
...
function disableNativeDownloadBehaviors() {
  document.querySelectorAll("a, area").forEach((element) => {
    if (controllerServices.isWormholesSafeDownloadElement(element)) return;
...
function installPrimarySafeControls() {
  installSafeControl(
    ...
    controllerServices.openUniverseTitleModal,
  );
  installSafeControl(
    document.getElementById("enterUniverseBtn"),
    controllerServices.openUniverseArchiveModal,
  );
  installSafeControl(
    document.getElementById("manageWormholesBtn"),
    controllerServices.openWormholesModal,
  );
}
...
function removeDownloadAttributesNear(element) {
  let current = element;
  while (current && current !== document.documentElement) {
    if (current.tagName === "A" && !controllerServices.isWormholesSafeDownloadElement(current)) {
    ...
    if (
      !controllerServices.isWormholesSafeDownloadElement(current) &&
...
function guardAgainstDownloadWrapper(event) {
  if (controllerServices.isWormholesSafeDownloadElement(event.target)) return;
...
function installSettingsMenuHandlers() {
  ...
  document
    ...
    ?.addEventListener("click", controllerServices.exportAppDataFromSettings);
  document
    .getElementById("importAppDataBtn")
    ?.addEventListener("click", controllerServices.importAppDataFromSettings);
  document
  ...
  document
    ...
    ?.addEventListener("click", controllerServices.openClearAppDataConfirmModal);
  document
    .getElementById("appDataImportInput")
    ?.addEventListener("change", controllerServices.handleAppDataImportFile);
  document
    .getElementById("cancelAppDataImportBtn")
    ?.addEventListener("click", () => controllerServices.closeAppDataImportConfirmModal(false));
  document
    .getElementById("confirmAppDataImportBtn")
    ?.addEventListener("click", () => controllerServices.closeAppDataImportConfirmModal(true));
  document
    .getElementById("cancelClearAppDataBtn")
    ?.addEventListener("click", controllerServices.closeClearAppDataConfirmModal);
  document
    .getElementById("confirmClearAppDataBtn")
    ?.addEventListener("click", controllerServices.proceedClearAppDataConfirm);
  document
    .getElementById("closeAppDataExportSummaryBtn")
    ?.addEventListener("click", controllerServices.closeAppDataExportSummaryModal);
  document
    .getElementById("createBackupBtn")
    ?.addEventListener("click", controllerServices.createBackupFromSettings);
  document
    .getElementById("restoreBackupBtn")
    ?.addEventListener("click", controllerServices.restoreBackupFromSettings);
  document
...
registerControllerServices(SETTINGS_CONTROLLER_API);
