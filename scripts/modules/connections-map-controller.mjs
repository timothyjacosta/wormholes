...
function renderConnectionsMapView() {
  ...
  if (
    selectedMapNodeId &&
    !controllerServices.isSelectableConnectionsMapNodeId(selectedMapNodeId)
  ) {
  ...
  const currentUniverse = controllerServices.getCurrentUniverse();
  ...
  function addCurrentUniverseNode() {
    ...
    addNode({
      ...
      title: controllerServices.getUniverseTitle(currentUniverseId),
  ...
  function addExternalUniverseNode(universeId) {
    ...
    addNode({
      ...
      id,
      universeId,
      title: controllerServices.getUniverseTitle(universeId),
  ...
  function addExternalCreationNode(universeId, creationId) {
    ...
    const groupEntry =
      entry && controllerServices.isGroupEntry(entry)
        ? entry
        : controllerServices.getGroupForEntryId(creationId, archive);

    if (groupEntry) {
      const groupNodeId = `external:${universeId}:${groupEntry.id}`;
      const children = controllerServices
        .groupChildIds(groupEntry)
      ...
      addNode({
        ...
        subtitle: controllerServices.getUniverseTitle(universeId),
        selectable: false,
        external: true,
      ...
      if (entry && controllerServices.isGroupEntry(entry)) {
        return groupNodeId;
      }

      if (entry && controllerServices.groupChildIds(groupEntry).includes(entry.id)) {
  ...
  function nodeDescriptorForId(nodeId, fallbackUniverseId = null, fallbackCreationId = null) {
    ...
    if (nodeShapes.has(nodeId) && controllerServices.getEntry(nodeId)) {
  ...
  function addBridgeEdge(
    ...
    const noteKey = controllerServices.bridgeNoteKeyForNodes(sourceNode, targetNode);
  ...
  function externalGroupLayout(node) {
    ...
    const groupFit = fitConnectionHeaderBox(
      node.title,
      node.subtitle || controllerServices.getUniverseTitle(node.universeId),
      {
        minWidth: 218,
        maxWidth: 350,
        minHeight: 58,
  ...
  const currentUniverseFit = fitConnectionHeaderBox(
    controllerServices.getUniverseTitle(currentUniverseId),
    "Working Universe",
    {
      minWidth: 220,
      maxWidth: 360,
  ...
  archiveEntries.forEach((entry) => {
    ...
    controllerServices.normalizeBridges(entry.bridges).forEach((bridge) => {
      ...
      addBridgeEdge(
        ...
        entry.title,
        bridge.creationId
          ? controllerServices.visibleEntryTitleForUniverseEntry(
              bridge.universeId,
              bridge.creationId,
            )
          : controllerServices.getUniverseTitle(bridge.universeId),
      );
    });
  });

  controllerServices.normalizeUniverseBridges(currentUniverse).forEach((bridge) => {
    addCurrentUniverseNode();
    const targetNodeId = bridge.creationId
      ? addExternalCreationNode(bridge.universeId, bridge.creationId)
      : addExternalUniverseNode(bridge.universeId);
    ...
    addBridgeEdge(
      ...
      controllerServices.getUniverseTitle(currentUniverseId),
      bridge.creationId
        ? controllerServices.visibleEntryTitleForUniverseEntry(bridge.universeId, bridge.creationId)
        : controllerServices.getUniverseTitle(bridge.universeId),
    );
  });
  ...
  universes.forEach((universe) => {
    ...
    controllerServices.normalizeUniverseBridges(universe).forEach((bridge) => {
      ...
      addBridgeEdge(
        ...
        bridge.creationId && nodeShapes.has(bridge.creationId)
          ...
          : {type: "universe", universeId: currentUniverseId},
        controllerServices.getUniverseTitle(universe.id),
        bridge.creationId && nodeShapes.has(bridge.creationId)
          ? controllerServices.visibleEntryTitleForUniverseEntry(
              currentUniverseId,
              bridge.creationId,
    ...
    readArchiveForUniverse(universe.id).forEach((externalEntry) => {
      controllerServices.normalizeBridges(externalEntry.bridges).forEach((bridge) => {
        if (bridge.universeId !== currentUniverseId) return;
        ...
        addBridgeEdge(
          ...
          bridge.creationId && nodeShapes.has(bridge.creationId)
            ? {type: "creation", universeId: currentUniverseId, creationId: bridge.creationId}
            : {type: "universe", universeId: currentUniverseId},
          controllerServices.visibleEntryTitleForUniverseEntry(universe.id, externalEntry.id),
          bridge.creationId && nodeShapes.has(bridge.creationId)
            ? controllerServices.visibleEntryTitleForUniverseEntry(
                currentUniverseId,
                bridge.creationId,
  ...
  const selectedEntry = selectedMapNodeId ? controllerServices.getEntry(selectedMapNodeId) : null;
  ...
  function connectionsMapNodeIdsForTargets(targets) {
    ...
    (safeTargets.entries || []).forEach((target) => {
      ...
      if (target.universeId === currentUniverseId) {
        ...
        const group =
          typeof controllerServices.getGroupForEntryId === "function"
            ? controllerServices.getGroupForEntryId(target.entryId, archiveEntries)
            : null;
  ...
  function resolveConnectionsMapSearchRecord(record) {
    ...
    return {
      nodeIds,
      targets,
      contextLabel: record.universeTitle || controllerServices.getUniverseTitle(record.universeId),
      nativeNodeId,
    };
  }
  ...
  function addConnectionNodeWithGroupContext(nodeId) {
    ...
    if (nodeId === selectedMapNodeId && node?.type === "group") {
      controllerServices.groupChildIds(node.entry).forEach((childId) => {
  ...
  const connectionClusterCandidates = [
    ...
    ...externalPlaceNodes.map((node) => {
      ...
      return {
        ...
        groupLabel: controllerServices.getUniverseTitle(node.universeId),
      };
    }),
  ].filter(Boolean);
  ...
  const bridgeEdgeData = bridgeEdges
    .map((edge) => {
      ...
      return {
        ...
        bx: clipped.bx,
        by: clipped.by,
        segmentId: `bridge:${controllerServices.bridgeNoteKeyForNodes(edge.sourceNode, edge.targetNode)}`,
      };
    })
    .filter(Boolean);
  ...
  const normalEdgeSvg = normalEdgeData
    .map((edge) => {
      ...
      const note = controllerServices.getConnectionNote(sourceId, targetId);
  ...
  const bridgeEdgeSvg = bridgeEdgeData
    .map((edge) => {
      ...
      const edgeClass = selectedMapNodeId ? (touchesSelected ? "highlighted" : "dimmed") : "";
      const bridgeNoteKey = controllerServices.bridgeNoteKeyForNodes(sourceNode, targetNode);
      const note = controllerServices.getBridgeNote(bridgeNoteKey);
      const safeNote = escapeHtml(truncatePreview(note, 34));
  ...
  function nodeStateClasses(node) {
    ...
    const isConnectable =
      ...
      !controllerServices.isExternalConnectionsMapNodeId(selectedMapNodeId) &&
      !controllerServices.isCurrentUniverseConnectionsMapNodeId(selectedMapNodeId) &&
      !isSelected &&
  ...
  function renderNodeBadge(node, shape) {
    ...
    const badgeCount =
      badgeType === "universe"
        ? controllerServices.literatureCountForUniverseTag(badgeUniverseId)
        : controllerServices.literatureCountForEntryTag(badgeUniverseId, badgeEntryId);

    const visionBadgeCount =
      badgeType === "universe"
        ? controllerServices.visionCountForUniverseTag(badgeUniverseId)
        : controllerServices.visionCountForEntryTag(badgeUniverseId, badgeEntryId);
  ...
  const currentUniverseNodeSvg =
    ...
    connectionNodeVisibleInCurrentView(currentUniverseNodeId)
      ? (() => {
          ...
          return `
        <g class="connection-node ${state.className} current-universe-center-node" data-id="${escapeHtml(currentUniverseNodeId)}" data-type="current-universe" style="${controllerServices.mapUniversePaletteStyle(currentUniverseId)}">
          ...
          ${connectionWrappedTextSvg(connectionNodeFits.get(currentUniverseNodeId) || {titleLines: [controllerServices.getUniverseTitle(currentUniverseId)], subtitleLines: ["Working Universe"], titleFontSize: 15, subtitleFontSize: 10.5, titleLineHeight: 16, subtitleLineHeight: 12}, currentUniverseShapeForRender.x, currentUniverseShapeForRender.y, {paddingX: 14, titleStartY: 24, titleWeight: "bold"})}
        </g>
      ...
      if (item.kind === "group") {
        ...
        const childrenSvg = visibleChildren
          .map((child) => {
            ...
            return `
          <g class="connection-node ${childState.className}" data-id="${escapeHtml(child.id)}" data-type="creation" style="${controllerServices.mapUniversePaletteStyle(currentUniverseId)}">
        ...
        return wrapConnectionClusterMember(
          item.entry.id,
          `
        <g class="connection-node ${state.className}" data-id="${escapeHtml(item.entry.id)}" data-type="creation" style="${controllerServices.mapUniversePaletteStyle(currentUniverseId)}">
      ...
      if (node.type === "external-group") {
        ...
        const childrenSvg = visibleChildren
          .map((child) => {
            ...
            const childState = nodeStateClasses(childNode);
            return `
          <g class="connection-node ${childState.className}" data-id="${escapeHtml(childNodeId)}" data-type="external" style="${controllerServices.mapUniversePaletteStyle(node.universeId || currentUniverseId)}">
            <rect x="${childShape.x}" y="${childShape.y}" width="${childShape.w}" height="${childShape.h}" rx="12" ry="12"></rect>
        ...
        return wrapConnectionClusterMember(
          node.id,
          `
        <g class="connection-node ${state.className}" data-id="${escapeHtml(node.id)}" data-type="external" style="${controllerServices.mapUniversePaletteStyle(node.universeId || currentUniverseId)}">
          <rect x="${shape.x}" y="${shape.y}" width="${shape.w}" height="${shape.h}" rx="18" ry="18"></rect>
          ${connectionWrappedTextSvg(connectionNodeFits.get(node.id) || {titleLines: [node.title || "Group"], subtitleLines: [node.subtitle || controllerServices.getUniverseTitle(node.universeId)], titleFontSize: 13.5, subtitleFontSize: 9.6, titleLineHeight: 15, subtitleLineHeight: 11}, shape.x, shape.y, {paddingX: 14, titleStartY: 24, titleWeight: "bold"})}
        </g>
        ${renderNodeBadge(node, shape)}
        ${childrenSvg}
      `,
  ...
  const selectedLocalMapEntry = selectedMapNodeId
    ? controllerServices.getEntry(selectedMapNodeId)
    : null;
  ...
  mapSearchApi?.bind("connections", {
    ...
    onSelect: (record, descriptor) => {
      ...
      if (
        ...
        controllerServices.isSelectableConnectionsMapNodeId(descriptor.nativeNodeId)
      ) {
  ...
  document.getElementById("mapBridgeBtn")?.addEventListener("click", (event) => {
    event.stopPropagation();
    if (controllerServices.isCurrentUniverseConnectionsMapNodeId(selectedMapNodeId)) {
      controllerServices.openUniverseBridgeModal(currentUniverseId);
      return;
    }
    if (selectedMapNodeId) controllerServices.openBridgeModal(selectedMapNodeId);
  });

  document.getElementById("clearMapSelectionBtn")?.addEventListener("click", (event) => {
    event.stopPropagation();
    controllerServices.clearMapSelection();
  });

  document.querySelectorAll(".connection-edge-group[data-source]").forEach((group) => {
    group.addEventListener("click", (event) => {
      event.stopPropagation();
      controllerServices.openConnectionModal(group.dataset.source, group.dataset.target);
    });
  });
  ...
  document.querySelectorAll(".bridge-note-edge").forEach((group) => {
    group.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      controllerServices.openBridgeNoteModal(group.dataset.bridgeKey, group.dataset.label);
    });
  });
  ...
  controllerServices.bindVisionBadgeClickHandlers(document.getElementById("connectionsMapWrap"));
  ...
  document.querySelectorAll(".connection-node").forEach((node) => {
    node.addEventListener("click", (event) => {
      ...
      const selectedExternal = controllerServices.isExternalConnectionsMapNodeId(selectedMapNodeId);
      ...
      if (clickedExternal) {
        ...
        if (selectedCurrentUniverse) {
          controllerServices.toggleUniverseBridgeToExternalNode(currentUniverseId, nodeId);
          return;
        }

        if (controllerServices.getEntry(selectedMapNodeId)) {
          controllerServices.toggleEntryBridgeToExternalNode(selectedMapNodeId, nodeId);
          return;
        }
      ...
      if (selectedExternal) {
        controllerServices.toggleEntryBridgeToExternalNode(nodeId, selectedMapNodeId);
        return;
      }
      ...
      controllerServices.toggleMapConnection(selectedMapNodeId, nodeId);
    });
  });

  controllerServices.bindVisionBadgeClickHandlers(document.getElementById("connectionsMapWrap"));
  controllerServices.protectAllControls();
...
registerControllerServices(CONNECTIONS_MAP_CONTROLLER_API);
