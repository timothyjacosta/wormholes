...
function buildWormholesMapListViewHtml() {
  ...
  const totalGroups = universes.reduce((sum, universe) => {
    return (
      sum +
      controllerServices
        .topLevelArchiveEntries(readArchiveForUniverse(universe.id))
        .filter(controllerServices.isGroupEntry).length
    );
  }, 0);
  const totalCreations = universes.reduce((sum, universe) => {
    return (
      ...
      controllerServices
        .topLevelArchiveEntries(readArchiveForUniverse(universe.id))
        .filter((entry) => !controllerServices.isGroupEntry(entry)).length
    );
  }, 0);
  ...
  const universeSections = universes
    .map((universe) => {
      ...
      const mapGroups = topEntries.filter(controllerServices.isGroupEntry);
      const mapCreations = topEntries.filter((entry) => !controllerServices.isGroupEntry(entry));
...
function renderWormholesMapView() {
  ...
  const systemBlueprints = universes.map((universe) => {
    ...
    const archive = controllerServices.mapArchiveEntries(fullArchive);
    ...
    const creationFits = archive.map((entry) => {
      if (controllerServices.isGroupEntry(entry)) {
        const childEntries = controllerServices
          .groupChildIds(entry)
  ...
  function markWormholeBridgeEntry(universeId, entryId) {
    ...
    const group = controllerServices.getGroupForEntryId(entryId, fullArchive);
  ...
  systems.forEach((system) => {
    ...
    controllerServices.normalizeUniverseBridges(system.universe).forEach((bridge) => {
      if (bridge.creationId) markWormholeBridgeEntry(bridge.universeId, bridge.creationId);
    });
  });
  ...
  function getWormholeDisplayId(universeId, entryId, fullArchive = null) {
    ...
    const mapped = controllerServices.mapEntryForIdInEntries(entryId, archive);
  ...
  const selectedEntry = selectedWormholeCreation
    ? controllerServices.getArchiveEntryFromUniverse(
  ...
  const selectedBridgeCreationTargets =
    selectedWormholeCreation && showWormholesBridges
      ? controllerServices.getCreationBridgeTargetsForCreation(
          selectedWormholeCreation.universeId,
          selectedWormholeCreation.creationId,
        )
      : new Set();
  const selectedBridgeUniverseTargets =
    selectedWormholeCreation && showWormholesBridges
      ? controllerServices.getUniverseBridgeTargetsForCreation(
          selectedWormholeCreation.universeId,
          selectedWormholeCreation.creationId,
        )
      : new Set();
  ...
  const focusedCreationContext = showWormholesBridges
    ? controllerServices.getCreationBridgeContextForUniverse(wormholeFocusUniverseId)
    : {externalTargets: new Set(), internalSources: new Set()};
  const focusedUniverseToCreationContext = showWormholesBridges
    ? controllerServices.getUniverseToCreationBridgeContextForFocus(wormholeFocusUniverseId)
  ...
  const focusedUniverseTargets = showWormholesBridges
    ? controllerServices.getUniverseBridgeTargetsForFocus(wormholeFocusUniverseId)
    : new Set();
  ...
  function addIsolatedWormholeCreation(universeId, creationId, options = {}) {
    ...
    const group =
      entry && controllerServices.isGroupEntry(entry)
        ? entry
        : controllerServices.getGroupForEntryId(creationId, fullArchive);
    ...
    const includeGroupMembers =
      ...
      (entry &&
        controllerServices.isGroupEntry(entry) &&
    ...
    : Math.ceil(
      ...
      systems.reduce(
        (sum, system) =>
          ...
          system.fullArchive.reduce(
            (entrySum, entry) =>
              entrySum + controllerServices.normalizeBridges(entry.bridges).length,
            0,
          ),
        0,
      ) +
      universes.reduce(
        (sum, universe) => sum + controllerServices.normalizeUniverseBridges(universe).length,
        0,
      );
  ...
  systems.forEach((system) => {
    ...
    system.fullArchive.forEach((entry) => {
      const mappedSource =
        controllerServices.mapEntryForIdInEntries(entry.id, system.fullArchive) || entry;
      ...
      (entry.connections || []).forEach((targetId) => {
        const mappedTarget = controllerServices.mapEntryForIdInEntries(
          targetId,
          system.fullArchive,
        ) || {id: targetId};
      ...
      controllerServices.normalizeBridges(entry.bridges).forEach((bridge) => {
        ...
        const noteKey = controllerServices.bridgeNoteKeyForNodes(
  ...
  universes.forEach((universe) => {
    ...
    controllerServices.normalizeUniverseBridges(universe).forEach((bridge) => {
      let targetPos = null;
      let targetNode = {type: "universe", universeId: bridge.universeId};
      let targetLabel = controllerServices.getUniverseTitle(bridge.universeId);
      ...
      if (bridge.creationId) {
        ...
        if (creationTarget) {
          ...
          targetLabel = controllerServices.visibleEntryTitleForUniverseEntry(
            bridge.universeId,
            bridge.creationId,
          );
        }
      ...
      if (!targetPos) {
        ...
        if (universeTarget) {
          ...
          targetLabel = controllerServices.getUniverseTitle(bridge.universeId);
        }
      }
      ...
      const note = controllerServices.getBridgeNote(noteKey);
      ...
      const targetIsGroupLevelBridge = !!(
        targetEntryForLevel && controllerServices.isGroupEntry(targetEntryForLevel)
      );
      const targetIsChildLevelBridge = !!(
        bridge.creationId &&
        targetArchiveForLevel &&
        controllerServices.getGroupForEntryId(bridge.creationId, targetArchiveForLevel)
      );
      ...
      universeBridgeLines.push(`
        <g class="wormhole-bridge-note-group" data-bridge-key="${escapeHtml(noteKey)}" data-label="${escapeHtml(`${controllerServices.getUniverseTitle(universe.id)} ↔ ${targetLabel}`)}">
  ...
  systems.forEach((system) => {
    ...
    system.fullArchive.forEach((entry) => {
      ...
      (entry.connections || []).forEach((targetId) => {
        ...
        const sourceIsGroupLevel = controllerServices.isGroupEntry(entry);
        const sourceIsChildLevel = !!controllerServices.getGroupForEntryId(
          entry.id,
          system.fullArchive,
        );
        const targetEntryForLevel = system.fullArchive.find((item) => item.id === targetId);
        const targetIsGroupLevel = controllerServices.isGroupEntry(targetEntryForLevel);
        const targetIsChildLevel = !!controllerServices.getGroupForEntryId(
          targetId,
          system.fullArchive,
        );
        const levelClass =
      ...
      controllerServices.normalizeBridges(entry.bridges).forEach((bridge) => {
        ...
        const mappedTargetKey = bridge.creationId
          ? `${bridge.universeId}:${(systems.find((item) => item.universe.id === bridge.universeId) ? controllerServices.mapEntryForIdInEntries(bridge.creationId, systems.find((item) => item.universe.id === bridge.universeId).fullArchive)?.id : bridge.creationId) || bridge.creationId}`
          : null;
        ...
        const sourceIsGroupLevelBridge = controllerServices.isGroupEntry(entry);
        const sourceIsChildLevelBridge = !!controllerServices.getGroupForEntryId(
          entry.id,
          system.fullArchive,
        );
        ...
        const targetIsGroupLevelBridge = !!(
          bridge.creationId &&
          targetArchiveForLevel &&
          controllerServices.isGroupEntry(
            targetArchiveForLevel.find((item) => item.id === bridge.creationId),
          )
        );
        const bridgeLevelClass =
        ...
        bridgeLines.push(`
          <g class="wormhole-bridge-note-group" data-bridge-key="${escapeHtml(noteKey)}" data-label="${escapeHtml(`${entry.title} ↔ ${bridge.creationId ? controllerServices.getCreationTitleFromUniverse(bridge.universeId, bridge.creationId) : controllerServices.getUniverseTitle(bridge.universeId)}`)}">
            ${bridgeVisibleSvg}
    ...
    .map((system) => {
      ...
      const creations = system.archive
        .map((entry) => {
          ...
          const className = [
            "wormhole-creation",
            controllerServices.isGroupEntry(entry) ? "wormhole-group-node" : "",
            isSelected ? "selected" : "",
      ...
      return `
      <g class="${clusterClass}" data-id="${escapeHtml(system.universe.id)}" style="${controllerServices.mapUniversePaletteStyle(system.universe.id)}">
  ...
  const wormholeSelectionTitle = selectedWormholeCreation
    ...
    : wormholeFocusUniverseId
      ? controllerServices.getUniverseTitle(wormholeFocusUniverseId)
      : "";
  ...
  const wormholeSelectionInstruction = selectedWormholeCreation
    ? `Select an item in another universe to add or remove a Bridge. Select another item in ${escapeHtml(controllerServices.getUniverseTitle(selectedWormholeCreation.universeId))} to add or remove a Connection.`
  ...
  document.getElementById("clearWormholeFocusBtn")?.addEventListener("click", (event) => {
    event.stopPropagation();
    controllerServices.clearWormholeFocus();
  });
  ...
  function handleWormholeSvgClick(event) {
    ...
    if (directAggregateCluster && directAggregateCluster.getAttribute("aria-hidden") !== "true") {
      ...
      controllerServices.swallowDownloadBehavior(event);
    ...
    if (directCreation) {
      event.__wormholeMapHandled = true;
      controllerServices.swallowDownloadBehavior(event);
      controllerServices.handleWormholeCreationClick(
    ...
    if (directUniverse) {
      event.__wormholeMapHandled = true;
      controllerServices.swallowDownloadBehavior(event);
      controllerServices.handleWormholeUniverseClick(directUniverse.dataset.id);
      return false;
    }
    ...
    if (target.type === "universe") {
      controllerServices.handleWormholeUniverseClick(target.universeId);
    } else {
      controllerServices.handleWormholeCreationClick(target.universeId, target.creationId);
    }
    return false;
  }
  ...
  document.querySelectorAll(".wormhole-cluster-title").forEach((node) => {
    controllerServices.installSafeControl(node, () => {
      if (!node.isConnected || wormholeIsolationActive) return;
      controllerServices.handleWormholeUniverseClick(node.dataset.id);
    });
  });
...
registerControllerServices(BRIDGES_MAP_CONTROLLER_API);
