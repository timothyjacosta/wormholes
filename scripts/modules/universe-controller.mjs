...
async function beginUniverseMutationJournal(options = {}) {
  ...
  if (
    typeof controllerServices.buildWormholesAppDataExport !== "function" ||
    !window.WormholesSnapshots?.createSnapshot
  ) {
    throw new Error("Universe recovery tools are unavailable.");
  }
  const rollbackData = await controllerServices.buildWormholesAppDataExport();
...
async function restoreUniverseMutationFromJournal(transaction) {
  ...
  if (typeof controllerServices.renderAfterWormholesAppDataImport === "function")
    await controllerServices.renderAfterWormholesAppDataImport();
...
function normalizeBridgeListForImport(bridges, sourceUniverseId = "", validUniverseIds = null) {
  ...
  return (Array.isArray(bridges) ? bridges : [])
    .map((bridge) => controllerServices.normalizeBridge(bridge))
    .filter((bridge) => {
      ...
      const key = controllerServices.bridgeKey(bridge.universeId, bridge.creationId);
  ...
  const normalized = {
    ...
    bridges: Array.isArray(universe?.bridges)
      ? validUniverseIds
        ...
        : controllerServices.normalizeUniverseBridges(universe)
      : [],
...
function runAppSchemaMigrations() {
  ...
  universes.forEach((universe) => {
    ...
    const archive = (Array.isArray(rawArchive) ? rawArchive : []).map(
      controllerServices.normalizeSchemaArchiveEntry,
    );
    ...
    controllerServices.writeLiteratureMetadataOnly(universe.id, literature);
    const vision = (Array.isArray(rawVision) ? rawVision : []).map((item) =>
      controllerServices.normalizeImportedVisionItem(item, universe.id),
    );
    controllerServices.writeVisionMetadataOnly(universe.id, vision);
  });
...
function stableUniverseFolderName(universe) {
  const titlePart =
    controllerServices
      .sanitizeFileNamePart(universe?.title || "Untitled Universe", "Untitled Universe")
      .slice(0, 56)
      .trim() || "Untitled Universe";
  return controllerServices.sanitizeFileNamePart(
...
function showHomeScreen() {
  ...
  controllerServices.closeLiteratureTagModal();
  controllerServices.closeLiteratureViewer();
  controllerServices.closeLiteratureUploadModal();
  controllerServices.closeVisionUploadModal();
  controllerServices.closeLiteratureLinksModal();
  closeMigrateModal();
  closeMigrateNewUniverseModal();
  closeCopyToUniverseModal?.();
  closeCopyNewUniverseModal?.();
  controllerServices.closeBridgeModal();
  controllerServices.closeBridgeNewUniverseModal();
  controllerServices.closeWormholesModal();
  closeUniverseSummaryModal();
  closeUniverseEditModal();
  controllerServices.closeDeleteEntryConfirm();
...
function createUniverseFromModal() {
  ...
  if (!title) {
    controllerServices.setModalErrorText("universeTitleError", "A universe title is required.");
    input.focus();
    return;
  }
  ...
  list.innerHTML = universes
    .map((universe) => {
      ...
      try {
        ...
        creationCount = archive.filter((entry) => !controllerServices.isGroupEntry(entry)).length;
        groupCount = archive.filter((entry) => controllerServices.isGroupEntry(entry)).length;
      } catch (e) {
        creationCount = 0;
        groupCount = 0;
      }

      const savedLiteratureEntries = controllerServices.readLiteratureForUniverse(universe.id);
      ...
      const savedImageCount = controllerServices.readVisionBoardForUniverse(universe.id).length;
      const metaParts = [
...
function saveUniverseEdit() {
  ...
  if (!title) {
    controllerServices.setModalErrorText("universeEditError", "A universe title is required.");
...
function cleanupBridgesToUniverse(universeId) {
  ...
  universes.forEach((universe) => {
    ...
    const cleaned = archive.map((entry) => {
      const oldBridges = controllerServices.normalizeBridges(entry.bridges);
    ...
    const oldUniverseBridges = controllerServices.normalizeUniverseBridges(universe);
  ...
  controllerServices.cleanupBridgeNotes();
...
async function deleteUniverseStorage(universeId, options = {}) {
  ...
  if (!deferCleanup && localFoldersEnabled && deletedUniverse) {
    await controllerServices.deleteUniverseFoldersFromDisk(deletedUniverse);
  }
  ...
  saveUniversesToStorage();
  if (!deferCleanup) await controllerServices.pruneWormholesFolderToAppState();
...
async function migrateAllAndDeleteUniverse(targetUniverseId) {
  ...
  const {idMap, migratedEntries} = controllerServices.cloneMigratedArchiveEntries(
    sourceArchive,
    idsToMigrate,
    sourceUniverse,
    targetUniverseId,
  ...
  if (window.WormholesDuplicateCreations?.reviewBatch) {
    ...
    if (duplicateReview.decision === "view") {
      ...
      switchTab("archive");
      controllerServices.revealArchiveEntryForTag(duplicateReview.match?.existing?.id || "");
      return;
    }
  ...
  try {
    ...
    controllerServices.remapIncomingBridgesForMigration(
      activeUniverseDeleteId,
      targetUniverseId,
      idMap,
    );
    controllerServices.remapBridgeNotesForMigratedEntries(
      activeUniverseDeleteId,
      targetUniverseId,
      idMap,
    );
...
async function createMigrateNewUniverse() {
  const sourceEntry = controllerServices.getEntry(activeMigrateEntryId);
  ...
  if (
    ...
    return;
  const migrationCount = controllerServices.isGroupEntry(sourceEntry)
    ? 1 + controllerServices.groupChildIds(sourceEntry).length
    : 1;
  if (
    window.WormholesEntityLimits &&
  ...
  closeMigrateNewUniverseModal();
  await controllerServices.migrateEntryToUniverse(universe.id);
  renderUniverseArchiveList();
}
...
async function saveMigratePickerModal() {
  ...
  await controllerServices.migrateEntryToUniverse(stagedMigrateTargetUniverseId);
}
...
registerControllerServices(UNIVERSE_CONTROLLER_API);
