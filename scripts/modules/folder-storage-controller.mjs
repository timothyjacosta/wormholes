...
async function requestFolderPermission(handle, mode = "readwrite") {
  ...
  try {
    ...
    await controllerServices.delay(40);
...
function appFolderFileMaps() {
  ...
  universes.forEach((universe) => {
    ...
    addValidFolderFile(
      literatureFolders,
      universe,
      controllerServices.readLiteratureForUniverse(universe.id),
    );
    addValidFolderFile(
      imageFolders,
      universe,
      controllerServices.readVisionBoardForUniverse(universe.id),
    );
  });
...
async function deleteUniverseFoldersFromDisk(universe) {
  ...
  const names = Array.from(
    new Set(
      [universeFolderName(universe), controllerServices.legacyUniverseFolderName(universe)].filter(
        Boolean,
      ),
    ),
  );
...
function repairFolderCollisionTitles() {
  ...
  (universes || []).forEach((universe) => {
    ...
    const docs = controllerServices.readLiteratureForUniverse(universe.id);
    if (repairFolderCollisionTitlesInList(docs)) {
      ...
      if (universe.id === currentUniverseId) {
        ...
        controllerServices.saveLiteratureForUniverse(universe.id, docs);
      }
    }

    const images = controllerServices.readVisionBoardForUniverse(universe.id);
    if (repairFolderCollisionTitlesInList(images)) {
      ...
      if (universe.id === currentUniverseId) {
        ...
        controllerServices.saveVisionBoardForUniverse(universe.id, images);
      }
    }
  });
...
async function blobFromVisionItemForDocx(row) {
  ...
  await controllerServices.materializeVisionItemLargeData(
    item,
  ...
  try {
    const src = await controllerServices.visionItemDisplaySrc(item, row.homeUniverseId);
    if (src) {
...
function linkedVisionRowsForCreationDocx(entry, universeId) {
  ...
  addRows(controllerServices.visionItemsForEntryTag(universeId, entry.id));

  if (controllerServices.isGroupEntry(entry)) {
    addRows(controllerServices.visionItemsForGroupChildrenTag(universeId, entry.id));
  }
...
async function docxImagesFromVisionRows(rows) {
  ...
  for (const row of rows || []) {
    const item = row.item;
    const label = `${item.title || item.sourceName || "Image"}${row.homeUniverseId ? ` (${controllerServices.getUniverseTitle(row.homeUniverseId)})` : ""}`;
    ...
    const thumbnailBlob = await controllerServices.imageBlobToThumbnailBlob(blob);
    if (!thumbnailBlob) {
      unavailable.push(`${label} — thumbnail unavailable`);
      continue;
    }
...
async function sourceUniverseFolderHandle(kind, universe) {
  ...
  const legacy = await getExistingDirectory(
    section,
    controllerServices.legacyUniverseFolderName(universe),
  );
...
registerControllerServices(FOLDER_STORAGE_CONTROLLER_API);
