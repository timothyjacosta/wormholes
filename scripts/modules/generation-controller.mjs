...
import importedThemeDeckApi from "./theme-decks.mjs";
...
function recordCompletedGenerationHistoryIfNeeded() {
  if (currentRollHistoryId || !currentGenerationSession || !isCurrentCreationComplete())
    return null;
  const universe =
    typeof controllerServices.getCurrentUniverse === "function"
...
function archivedGenerationDiagnostics(entryOrId) {
  const entry =
    typeof entryOrId === "string"
      ? typeof controllerServices.getEntry === "function"
        ? controllerServices.getEntry(entryOrId)
        : null
      : entryOrId;
  const metadata = entry?._generation;
  if (!metadata || typeof metadata !== "object") return null;
  const normalized =
    typeof controllerServices.normalizeGenerationDiagnostics === "function"
      ? controllerServices.normalizeGenerationDiagnostics(metadata)
      : metadata;
...
function updateManualButtons() {
  ...
  controllerServices.setAppButtonDisabled(document.getElementById("clearManualBtn"), !hasData);
  controllerServices.setAppButtonDisabled(
    saveManualBtn,
  ...
  if (typeof controllerServices.syncAllAppButtonStates === "function")
    controllerServices.syncAllAppButtonStates();
}
...
async function saveManualCreation() {
  ...
  if (!controllerServices.entryHasArchivableCreationData(entryValues)) {
  ...
  await controllerServices.writeArchiveEntryToFolderIfNeeded(entry);
...
registerControllerServices(GENERATION_CONTROLLER_API);
