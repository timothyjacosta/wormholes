/* GENERATED from scripts/modules/connections-controller.mjs. Do not edit this direct-file compatibility adapter. */
...
function renderConnectStatus() {
  ...
  const source = (globalThis.controllerServices || globalThis).getEntry(connectSourceId);
  if (!source) {
...
function openConnectPickerModal(entryId) {
  ...
  stagedConnectTargetIds = new Set(
    (entry.connections || []).filter((id) => id !== entryId && (globalThis.controllerServices || globalThis).getEntry(id)),
  );
...
function renderConnectEntryPicker(entry, depth = 0) {
  const source = (globalThis.controllerServices || globalThis).getEntry(activeConnectEntryId);
  const isSource = source && source.id === entry.id;
  const isGroup = (globalThis.controllerServices || globalThis).isGroupEntry(entry);
  const key = (globalThis.controllerServices || globalThis).nestedPickerKey("connect-group", currentUniverseId, entry.id);
    ...
    <div class="nested-picker-node depth-${depth}">
      <div class="nested-picker-row ${alreadyConnected ? "selected" : ""} ${isSource ? "disabled" : ""}">
        <button class="nested-picker-select app-button" type="button" data-app-button="true" data-picker-action="connect-entry" data-entry-id="${escapeHtml(entry.id)}" aria-pressed="${alreadyConnected ? "true" : "false"}" aria-label="${escapeHtml(`${alreadyConnected ? "Disconnect" : "Connect"} ${isGroup ? "group" : "creation"}: ${entry.title}${isSource ? ". Current item; cannot connect to itself" : ""}`)}" ${isSource ? `aria-disabled="true"` : ""}>
          ...
          <span class="group-choice-meta">${escapeHtml((globalThis.controllerServices || globalThis).entryPickerMeta(entry))} · ${escapeHtml(meta)}</span>
        </button>
    ...
    <div class="nested-picker-universe">
      <div class="nested-picker-row selected">
        <span class="nested-picker-select locked">
          <span class="group-choice-title">${escapeHtml((globalThis.controllerServices || globalThis).getCurrentUniverse()?.title || "Current Universe")}</span>
...
function saveConnectPickerModal() {
  ...
  const original = new Set(
    (source.connections || []).filter((id) => id !== source.id && (globalThis.controllerServices || globalThis).getEntry(id)),
  );
  const staged = new Set(
    Array.from(stagedConnectTargetIds).filter(
      (id) => id !== source.id && (globalThis.controllerServices || globalThis).getEntry(id),
    ),
  );
  ...
  original.forEach((targetId) => {
    ...
    const target = (globalThis.controllerServices || globalThis).getEntry(targetId);
...
function openConnectionModal(sourceId, targetId) {
  ...
  const source = (globalThis.controllerServices || globalThis).getEntry(sourceId);
  const target = (globalThis.controllerServices || globalThis).getEntry(targetId);
...
function toggleMapConnection(sourceId, targetId) {
  ...
  if (alreadyConnected) {
    openRelationshipRemovalConfirm("connection", source.title, target.title, () => {
      (globalThis.controllerServices || globalThis).connectEntries(sourceId, targetId, {silentToast: true});
...
function connectionsMapNodeTitle(nodeId) {
  const localEntry = (globalThis.controllerServices || globalThis).getEntry(nodeId);
  ...
  if (target.type === "universe") {
    return (globalThis.controllerServices || globalThis).getUniverseTitle(target.universeId);
  }

  return (
    (globalThis.controllerServices || globalThis).visibleEntryTitleForUniverseEntry(target.universeId, target.creationId) ||
    (globalThis.controllerServices || globalThis).getCreationTitleFromUniverse(target.universeId, target.creationId) ||
    "Linked Creation"
  );
}
...
function isSelectableConnectionsMapNodeId(nodeId) {
  ...
  if ((globalThis.controllerServices || globalThis).getEntry(nodeId)) return true;
  ...
  if (target.type === "creation") {
    return (globalThis.controllerServices || globalThis).entityExistsInUniverse(target.universeId, target.creationId);
  }
...
(globalThis.registerControllerServices || (() => {}))(CONNECTIONS_CONTROLLER_API);
